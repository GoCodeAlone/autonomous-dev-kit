...
| **Declared integration proof** | If the design says a component, package, plugin, module, connector, auth provider, storage backend, worker, scheduler, webhook, UI contribution, CLI extension, SDK adapter, or other external integration is installed/enabled/used, does it define an integration matrix with every declared item marked `config-only`, `runtime-integrated`, or `deferred`? For runtime-integrated items, does it prove behavior through the real host/consumer rather than only the provider package? Installation, lockfile changes, config presence, and provider unit tests are not enough. |
| **Contributed UI rendering proof** | If the design says a plugin registers UI into a host shell (admin panel, dashboard card, page, widget, route, navigation item), does it require proof beyond metadata/API existence: provider contribution output, host listing/authorization for a real principal, shell navigation, rendered non-empty contribution-specific content, and unauthorized denial? A design that only checks the provider contract or route registration is one-sided. |
...
| **Missing declared integration matrix** | For any declared integration in manifests, config, lockfiles, dependencies, deployment descriptors, or the plan itself, does the plan enumerate it and mark it `config-only`, `runtime-integrated`, or `deferred`? Runtime-integrated rows need a launched-host/consumer proof task that exercises a representative lifecycle; config-only/deferred rows need rationale or tracking. Flag plans that stop at install/update/config steps. |
| **Missing contributed UI route proof** | For plugin/admin/UI contribution work, does the plan launch the host and load each new contributed route through the shell with a real session? Flag plans that stop at provider metadata, host contribution JSON, static route registration, or screenshots of shell chrome without contribution-specific content assertions. |
...
| **Planned-code compile-validity** | When the plan **embeds Go (or other compiled) code** in prose/code-fences (struct definitions, helper functions, loop constructs), that code must be **compile-valid**, not just plausible. Two recurring slip classes: (a) **range-over-slice** written as `for t := range someSlice` (t is an int index, not the element) — must be `for _, t := range`; (b) **struct comparability** — structs containing slice/map/function fields (e.g. `FragmentSpec{ Fields []string }`) cannot be compared with `==`/`!=`; compare field-by-field. Eyeballing control flow is not enough; mentally (or actually) compile the embedded snippet. Flag any embedded code that would not compile. (2× trend: capability-assemble retro #1's range-over-slice P1 + scaffold-platform M1's `*Fragment != *Fragment` slip — both passed plan review as "embedded code clean" on inspection alone.) |
