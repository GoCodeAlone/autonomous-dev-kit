...
if echo "$output" | grep -qi "autodev:brainstorming\|autodev:using-autodev\|Available skills"; then
    echo "  [PASS] find_skills tool discovered autodev skills"
else
...
# Test 3: Test use_skill with autodev: prefix
echo ""
echo "Test 3: Testing use_skill with autodev: prefix..."
echo "  Running opencode with autodev:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load autodev:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded autodev:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load autodev:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
