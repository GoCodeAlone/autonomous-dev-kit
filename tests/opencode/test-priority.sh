...
# Verifies that skills are resolved with correct priority: project > personal > autodev
...
# 1. Create in autodev location (lowest priority)
mkdir -p "$HOME/.config/opencode/autodev/skills/priority-test"
cat > "$HOME/.config/opencode/autodev/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Autonomous Dev Kit version of priority test skill
---
# Priority Test Skill (Autonomous Dev Kit Version)
...
if [ -f "$HOME/.config/opencode/autodev/skills/priority-test/SKILL.md" ]; then
    echo "  [PASS] Autonomous Dev Kit version exists"
else
    echo "  [FAIL] Autonomous Dev Kit version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides autodev
echo ""
echo "Test 2: Testing personal > autodev priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides autodev)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [FAIL] Autonomous Dev Kit version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|autodev" | head -10
fi

# Test 3: Test that project overrides both personal and autodev
echo ""
echo "Test 3: Testing project > personal > autodev priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Autonomous Dev Kit version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit autodev: prefix bypasses priority
echo ""
echo "Test 4: Testing autodev: prefix forces autodev version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load autodev:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] autodev: prefix correctly forces autodev version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] autodev: prefix did not force autodev version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
