...
**Goal:** Add full autodev support for OpenCode.ai with a native JavaScript plugin that shares core functionality with the existing Codex implementation.
...
- Reference: `.codex/autodev-codex` (lines 40-74)
...
- Reference: `.codex/autodev-codex` (lines 97-136)
 ...
 * @param {string} sourceType - 'personal' or 'autodev' for namespacing
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/autodev-codex` (lines 212-280)
 ...
 * (personal skills override autodev skills).
 *
 * @param {string} skillName - Name like "autodev:brainstorming" or "my-skill"
 * @param {string} autodevDir - Path to autodev skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, autodevDir, personalDir) {
    // Strip autodev: prefix if present
    const forceAutodev = skillName.startsWith('autodev:');
    const actualSkillName = forceAutodev ? skillName.replace(/^autodev:/, '') : skillName;

    // Try personal skills first (unless explicitly autodev:)
    if (!forceAutodev && personalDir) {
    ...
    // Try autodev skills
    if (autodevDir) {
        const autodevPath = path.join(autodevDir, actualSkillName);
        const autodevSkillFile = path.join(autodevPath, 'SKILL.md');
        if (fs.existsSync(autodevSkillFile)) {
            return {
                skillFile: autodevSkillFile,
                sourceType: 'autodev',
                skillPath: actualSkillName
            };
        }
...
**Files:**
- Modify: `lib/skills-core.js`
- Reference: `.codex/autodev-codex` (lines 16-38)
...
**Files:**
- Modify: `.codex/autodev-codex` (add import at top)
...
Run: `node -c .codex/autodev-codex`
Expected: No output

**Step 3: Commit**

```bash
git add .codex/autodev-codex
...
**Files:**
- Modify: `.codex/autodev-codex` (lines 40-74)
...
Run: `.codex/autodev-codex find-skills | head -20`
...
**Files:**
- Modify: `.codex/autodev-codex` (lines 97-136, approximately)
...
**Files:**
- Modify: `.codex/autodev-codex` (lines 16-38, approximately)
...
Run: `.codex/autodev-codex bootstrap | head -50`
...
**Files:**
- Create: `.opencode/plugin/autodev.js`
...
const autodevSkillsDir = path.join(homeDir, '.config/opencode/autodev/skills');
const personalSkillsDir = path.join(homeDir, '.config/opencode/skills');

/**
 * OpenCode plugin entry point
 */
export const AutodevPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
...
Run: `ls -l .opencode/plugin/autodev.js`
Expected: File exists

**Step 4: Commit**

```bash
git add .opencode/plugin/autodev.js
...
**Files:**
- Modify: `.opencode/plugin/autodev.js`
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "autodev:brainstorming" or "my-custom-skill")')
        }),
        execute: async ({ skill_name }) => {
          // Resolve skill path (handles shadowing: personal > autodev)
...
Run: `node -c .opencode/plugin/autodev.js`
Expected: No output
...
```javascript
      {
        name: 'find_skills',
        description: 'List all available skills in the autodev and personal skill libraries.',
        schema: z.object({}),
        execute: async () => {
          // Find skills in both directories
          const autodevSkills = skillsCore.findSkillsInDir(
            autodevSkillsDir,
            'autodev',
            3
          );
          ...
          if (allSkills.length === 0) {
            return 'No skills found. Install autodev skills to ~/.config/opencode/autodev/skills/';
          }

          let output = 'Available skills:\n\n';

          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'autodev:';
...
```javascript
    'session.started': async () => {
      // Read using-autodev skill content
      const usingAutodevPath = skillsCore.resolveSkillPath(
        'using-autodev',
        autodevSkillsDir,
        personalSkillsDir
      );

      let usingAutodevContent = '';
      if (usingAutodevPath) {
        const fullContent = fs.readFileSync(usingAutodevPath.skillFile, 'utf8');
        // Strip frontmatter
        const lines = fullContent.split('\n');
        let inFrontmatter = false;
        let frontmatterEnded = false;
        ...
        usingAutodevContent = contentLines.join('\n').trim();
      }
...
**Skills naming:**
- Autonomous Dev Kit skills: \`autodev:skill-name\` (from ~/.config/opencode/autodev/skills/)
...
`;
      ...
      const hasUpdates = skillsCore.checkForUpdates(
        path.join(homeDir, '.config/opencode/autodev')
      );

      const updateNotice = hasUpdates ?
        '\n\n⚠️ **Updates available!** Run `cd ~/.config/opencode/autodev && git pull` to update autodev.' :
        '';
...
**Below is the full content of your 'autodev:using-autodev' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
### 1. Install Autonomous Dev Kit Skills

```bash
# Clone autodev skills to OpenCode config directory
mkdir -p ~/.config/opencode/autodev
git clone https://github.com/GoCodeAlone/autonomous-dev-kit.git ~/.config/opencode/autodev
```

### 2. Install the Plugin

The plugin is included in the autodev repository you just cloned.

OpenCode will automatically discover it from:
- `~/.config/opencode/autodev/.opencode/plugin/autodev.js`
...
ln -s ~/.config/opencode/autodev/.opencode/plugin/autodev.js .opencode/plugin/autodev.js
```
...
```
use use_skill tool with skill_name: "autodev:brainstorming"
```
...
1. Check plugin file exists: `ls ~/.config/opencode/autodev/.opencode/plugin/autodev.js`
2. Check OpenCode logs for errors
3. Verify Node.js is installed: `node --version`

### Skills not found

1. Verify skills directory exists: `ls ~/.config/opencode/autodev/skills`
...
- Documentation: https://github.com/GoCodeAlone/autonomous-dev-kit
```
...
Autonomous Dev Kit works with [OpenCode.ai](https://opencode.ai) through a native JavaScript plugin.
...
Run: `.codex/autodev-codex use-skill autodev:brainstorming | head -20`
Expected: Shows brainstorming skill content

**Step 3: Test bootstrap command**

Run: `.codex/autodev-codex bootstrap | head -30`
...
ls -l .opencode/plugin/autodev.js
...
- Total commits made
- Files created: `lib/skills-core.js`, `.opencode/plugin/autodev.js`, `.opencode/INSTALL.md`
- Files modified: `.codex/autodev-codex`, `README.md`, `RELEASE-NOTES.md`
...
- [ ] `.codex/autodev-codex` refactored to use shared core
- [ ] Codex commands still work (find-skills, use-skill, bootstrap)
- [ ] `.opencode/plugin/autodev.js` created with tools and hooks
