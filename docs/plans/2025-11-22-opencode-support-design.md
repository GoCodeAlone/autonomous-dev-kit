...
Add full autodev support for OpenCode.ai using a native OpenCode plugin architecture that shares core functionality with the existing Codex implementation.

## Background

OpenCode.ai is a coding agent similar to Claude Code and Codex. Previous attempts to port autodev to OpenCode (PR #93, PR #116) used file-copying approaches. This design takes a different approach: building a native OpenCode plugin using their JavaScript/TypeScript plugin system while sharing code with the Codex implementation.
...
2. **Platform-Specific Wrappers**
   - Codex: CLI script (`.codex/autodev-codex`)
   - OpenCode: Plugin module (`.opencode/plugin/autodev.js`)

3. **Skill Directories**
   - Core: `~/.config/opencode/autodev/skills/` (or installed location)
   - Personal: `~/.config/opencode/skills/` (shadows core skills)

### Code Reuse Strategy

Extract common functionality from `.codex/autodev-codex` into shared module:
...
```javascript
{
  ...
  schema: z.object({
    skill_name: z.string().describe('Name of skill (e.g., "autodev:brainstorming")')
  }),
...
1. **Inject using-autodev content**
   - Full content of the using-autodev skill
...
export const AutodevPlugin = async ({ client, directory, $ }) => {
  const autodevDir = path.join(process.env.HOME, '.config/opencode/autodev');
  const personalDir = path.join(process.env.HOME, '.config/opencode/skills');

  return {
    'session.started': async () => {
      const usingAutodev = await readSkill('using-autodev');
      const skillsList = await findAllSkills();
      const toolMapping = getToolMappingInstructions();

      return {
        context: `${usingAutodev}\n\n${skillsList}\n\n${toolMapping}`
      };
    },
...
├── .codex/
│   ├── autodev-codex        # UPDATED: Use skills-core
│   ├── autodev-bootstrap.md
│   └── INSTALL.md
├── .opencode/
│   ├── plugin/
│   │   └── autodev.js       # NEW: OpenCode plugin
...
1. Create `lib/skills-core.js`
   - Extract frontmatter parsing from `.codex/autodev-codex`
...
2. Update `.codex/autodev-codex` to use shared core
...
1. Create `.opencode/plugin/autodev.js`
